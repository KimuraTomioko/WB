...
try:
    ...
    # Создание заголовков столбцов в новом Excel файле
        ...
        for card in product_cards:
            try:
                ...
                if brand_to_search.lower() in brand_element.lower():
                    name_element = card.find_element(By.CLASS_NAME, 'product-card__name').text.strip()  # Название товара
                    
                    # Извлекаем цены
                    price_element = card.find_element(By.CSS_SELECTOR, '.price .price__lower-price').text.strip()  # Цена со скидкой
                    discounted_price_element = card.find_element(By.CSS_SELECTOR, '.price del').text.strip()  # Цена без скидки
                    
                    article_id = card.get_attribute('data-nm-id')  # Артикул товара

                    # Записываем данные в Excel файл
                    output_sheet.append([brand_element, name_element, discounted_price_element, price_element, article_id])

            except Exception as e:
                print(f"Exception occurred while parsing card: {e}")

        # Сохраняем результаты после обработки всех карточек для одной строки входного Excel файла
        clean_brand = clean_filename(brand_to_search)
        clean_name = clean_filename(name)
        output_wb.save(f"output_{clean_brand}_{clean_name}.xlsx")  # Сохраняем файл с уникальным именем
